...
import Proto.compiled.SendLikes_pb2
...
def send_like(server_url, auth_token, target_uid, target_region):
    """
    Send a like to a player's profile.

    Args:
        server_url (str): Base URL of the API server.
        auth_token (str): Bearer token used for authentication.
        target_uid (int): Target player account ID.
        target_region (str): Target player region (e.g., "IND").

    Returns:
        bool: True if the like was sent successfully.
    """
    ...
    }, Proto.compiled.SendLikes_pb2.like())
    ...
    if DEBUG:
        print("[SendLike] Response(raw):", response.content, "\n")
    response.raise_for_status()
    return True
