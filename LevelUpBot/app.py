...
async def fetch_player_info(session, uid):
    url = f"https://infoooooo-v6v5.vercel.app/accinfo?uid={uid}&region=ind"
...
[B][C][FFFFFF]Status: Online & Active"""
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = """[B][C][FFFFFF]Welcome to NAJMI Bot

[B][C][FFFFFF]Use Autoclicker 

[B][C][FFFFFF]Use /help to see all commands

[B][C][FFFFFF]Telegram : @najmiffexperimentv2
[B][C][FFFFFF]Contact : @NAJMI_ADMIN
...
[C][B][FFFFFF] STAR GAMER

[B][C][FFFFFF]Telegram : @STAR_METHODE
[B][C][FFFFFF]Contact : @STAR_GMR
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 3:
                                ...
                                await dl_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                try:
                                    success = await Room_Spam(target_uid, room_id, "STAR GAMER", key, iv)
                                    if success:
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                ...
                                await dl_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.strip().lower() == "NAJMI FF EXPERIMENT  bhai mera uid do":
...
[FFFFFF]├──────────┤
[FFFFFF] Developer  → NAJMI FF EXPERIMENT 
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 NAJMI FF EXPERIMENT BOT - INITIALIZING")
    print("=" * 50)
    ...
    # ---------- 加载动画（模仿第二个版本）----------
    os.system('clear')
    print("🤖 NAJMI FF EXPERIMENT  BOT - STARTING")
    print("=" * 50)
    for i in range(1, 4):
        dots = "." * i
    ...
    os.system('clear')
    print("🤖 NAJMI FF EXPERIMENT  BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
    ...
    os.system('clear')
    print(render('STAR', colors=['white', 'green'], align='center'))
    print('')
    print("🤖 NAJMI FF EXPERIMENT  BOT - ONLINE")
    print("=" * 50)
    print(f"🔹 UID: {TarGeT}")
